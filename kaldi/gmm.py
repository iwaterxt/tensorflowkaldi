...
class GMM(object):
    ...
    def align(self):
        ...
        #do the alignment to training data
        os.system('''steps/align_si.sh --nj %s --cmd %s --config %s/config/ali_%s %s %s %s %s_ali''' % (
                ...
                self.conf.get('directories', 'expdir') + '/' + self.name,
                self.conf.get('directories', 'expdir') + '/' + self.name ))
        #do the alignment to dev data

        os.system('''steps/align_si.sh --nj %s --cmd %s --config %s/config/ali_%s %s %s %s %s_dev_ali''' % (
                self.conf.get('general', 'num_jobs'),
                self.conf.get('general', 'cmd'), current_dir, self.conf_file,
                self.conf.get('directories', 'dev_features') + '/'
                + self.conf.get('gmm-features', 'name'),
                self.conf.get('directories', 'language'),
                self.conf.get('directories', 'expdir') + '/' + self.name,
                self.conf.get('directories', 'expdir') + '/' + self.name ))

        #convert training data alignments (transition-ids) to pdf-ids
        for i in range(int(self.conf.get('general', 'num_jobs'))):
            os.system('''gunzip -c %s_ali/ali.%d.gz | ali-to-pdf %s/final.mdl ark:- ark,t:- | gzip >  %s_ali/pdf.%d.gz''' % (
                    self.conf.get('directories', 'expdir') + '/' + self.name,
                    i+1, self.conf.get('directories', 'expdir') + '/'
                    + self.name, self.conf.get('directories', 'expdir')
                    + '/' + self.name, i+1))

        #convert dev data alignments (transition-ids) to pdf-ids
        for i in range(int(self.conf.get('general', 'num_jobs'))):
            os.system('''gunzip -c %s_dev_ali/ali.%d.gz | ali-to-pdf %s/final.mdl ark:- ark,t:- | gzip >  %s_dev_ali/pdf.%d.gz''' % (
                    self.conf.get('directories', 'expdir') + '/' + self.name,
                    i+1, self.conf.get('directories', 'expdir') + '/'
                    + self.name, self.conf.get('directories', 'expdir')
                    + '/' + self.name, i+1))
